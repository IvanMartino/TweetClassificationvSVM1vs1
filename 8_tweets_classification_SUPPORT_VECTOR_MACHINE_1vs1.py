...
using Support Vector Machine with decision_function_shape='ovo'
...
def main():
    ...
    print("-----------------------------")
    print("METHOD - MULTI-CLASS SUPPORT VECTOR MACHINE - 1 vs 1")
    print("-----------------------------")


    print("Hello,\nwe will use a SVM one versus one \nto classify twitter users\n")
    setpath()
    ...
    model_MCSVM = svm.SVC(decision_function_shape='ovo', probability=True)
    model_MCSVM.fit(X_train[0:m], Y_train[0:m])

    #clf.predict(X_dev)
    acc=model_MCSVM.score(X_train[0:m], Y_train[0:m])
    acc_dev=model_MCSVM.score(X_dev, Y_dev)
    print("Accuracy on the training set", acc)
    print("Accuracy on the development set", acc_dev)


    df_score, df_fp, df_pre=compute_accuracies(model_MCSVM,1,screen_names, X_train[:,0:n_features], X_dev[:,0:n_features], Y_train, Y_dev)
